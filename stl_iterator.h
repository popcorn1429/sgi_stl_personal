...
//This is the old version of reverse_iterator, as found in the 
// original HP STL. It does not use partial specialization.
#ifndef __STL_LIMITED_DEFAULT_TEMPLATES
...
template <class _RandomAccessIterator, class _Tp, class _Reference, class _Distance>
inline reverse_iterator<_RandomAccessIterator, _Tp, _Reference, _Distance> operator+(_Distance __n, const reverse_iterator<_RandomAccessIterator, _Tp, _Reference, _Distance>& __x) {
    reverse_iterator<_RandomAccessIterator, _Tp, _Reference, _Distance>(__x.base() - __n);
}
...
//We do not read any characters until operator* is called.
//The first time operator* is called, it calls getc.
//Subsequent calls to getc return a cached character, and 
//calls to operator++ use snextc. Before operator* or 
//operator++ has been called, _M_is_initialized is false.
...
public:
    ...
    istreambuf_iterator(istream_type& __istr) {
        _M_init(__istr.rdbuf());
    }
    ...
    bool equal(const istreambuf_iterator& __i) const {
        return _M_is_initialized && __i._M_is_initialized ? _M_eof == __i._M_eof : _M_equal_aux(__i);
    }
...
public:
    ...
    ostreambuf_iterator(ostream_type& __o) : _M_buf(__o.rdbuf()), _M_ok(__o.rdbuf() != nullptr) {}
...
template <class _Tp, class _Dist>
inline bool operator==(const istream_iterator<_Tp, _Dist>& __x, const istream_iterator<_Tp, _Dist>& __y) {
...
public:
    ...
    ostream_iterator(ostream& __s) : _M_stream(&__s), _M_string(nullptr) {}
